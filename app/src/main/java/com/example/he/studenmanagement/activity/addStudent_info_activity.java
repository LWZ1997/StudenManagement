                ...
                if (!TextUtils.isEmpty(id_) && !TextUtils.isEmpty(name_) && !TextUtils.isEmpty(sex_)) {
                    ...
                    if (sex_.matches("[女|男]")) {
                        ...
                        if (cursor.moveToNext()) {
                            ...
                            db.execSQL("insert into student(id,name,sex,password,number,mathScore) values(?,?,?,?,?,?)", new String[]{id_, name_, sex_, password_, number_, mathScore});
                            db.setTransactionSuccessful();//事务执行成功
                            db.endTransaction();//结束事务
                            Intent intent = new Intent(addStudent_info_activity.this, admin_manage_activity.class);
